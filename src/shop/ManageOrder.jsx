...
const ManageOrder = () => {
  ...
  const getManageOrder = async () => {
    try {
      const response = await axiosInstance.get('/shop/shop/manageOrder', {
        headers: {
...
const handleUpdate = async (orderId) => {
  ...
  try {
    const response = await axiosInstance.put(`/shop/shop/updateOrder/${orderId}`,
      {newStatus:newStatus},
      {
        headers: {
          authorization:`Bearer ${authStore.accessToken}`
    ...
    <div className="p-6">
      ...
      <div
            ...
            >
                ...
                <span className='w-1/6 text-center pl-24'>Status</span>
            </div>
        {filteredOrders.length > 0 ? (
          filteredOrders.map((item) => (            
            ...
            >              
              ...
              <p className="text-sm pl-11 w-1/6">${item.totalAmount}</p>
           ...
           <div>
              ...
              {updatedStatus[item._id] && updatedStatus[item._id] !== item.status && (
                <button
                  className="ml-5 mt-3 py-1 px-4 bg-blue-500 text-white rounded-full hover:bg-blue-600"
